# PROJECT MODUL 5: DEEP LEARNING UNTUK KOMPUTER VISION
...
10. **Semantic Segmentation Fine-tuner** — Fine-tune DeepLab untuk segmentasi domain spesifik (medical/satellite).
11. **Video Object Detection Pipeline** — Deteksi + tracking objek pada video panjang.
12. **Mobile-optimized Classifier** — MobileNet + quantization untuk deployment di mobile.
13. **Few-shot Learning Demo** — Klasifikasi kelas baru dengan hanya 5 contoh.
14. **Auto-annotation Tool** — Gunakan model pre-trained untuk auto-generate anotasi training data.
15. **Model Explainability Dashboard** — Tooltip GradCAM + confusion matrix + per-class metrics.
...
Pemerintah kota ingin memasang mesin sortir sampah di tempat umum yang menggunakan kamera untuk mengklasifikasikan jenis sampah (organik, plastik, kertas, logam, kaca). Buatlah: (a) dataset 5 kelas (kumpulkan 50+ gambar per kelas dari internet), (b) train classifier (transfer learning), (c) data augmentation, (d) deteksi objek sampah di atas conveyor belt (YOLO), (e) real-time classification dari webcam, (f) statistik jumlah per jenis, (g) export model ke ONNX.

### Soal 2: Asisten Pengemudi (Driver Assistance)
Startup otomotif memerlukan prototipe asisten pengemudi. Buatlah: (a) deteksi kendaraan + pejalan kaki (YOLO), (b) segmentasi jalan (semantic segmentation), (c) estimasi jarak (berdasarkan ukuran bounding box), (d) alert jika pejalan kaki terlalu dekat, (e) real-time processing dari video dashcam, (f) logging event berbahaya, (g) statistik per trip.

### Soal 3: Sistem Monitoring Kehadiran Kelas
Kampus ingin memantau kehadiran otomatis dari CCTV kelas. Buatlah: (a) deteksi wajah/orang (YOLO), (b) counting jumlah mahasiswa per frame, (c) instance segmentation untuk memisahkan tiap orang, (d) grafik kehadiran per waktu, (e) deteksi posisi duduk (kursi terisi/kosong), (f) real-time dari webcam, (g) export laporan.

### Soal 4: Aplikasi Identifikasi Spesies Tanaman
Kebun raya memerlukan aplikasi untuk pengunjung mengidentifikasi tanaman dari foto. Buatlah: (a) dataset 10 spesies tanaman (download dari internet), (b) transfer learning (MobileNet/ResNet), (c) augmentasi fotografi outdoor, (d) GradCAM untuk menunjukkan area identifikasi, (e) top-3 prediksi dengan confidence, (f) informasi singkat per spesies, (g) interface sederhana.

### Soal 5: Quality Control Defect Detection
Pabrik elektronik memerlukan sistem deteksi cacat pada PCB. Buatlah: (a) dataset gambar PCB normal dan cacat (atau pakai MVTec AD dataset), (b) train model deteksi cacat, (c) segmentasi area cacat, (d) klasifikasi jenis cacat (scratch, missing component, misalignment), (e) confidence scoring, (f) visualisasi heatmap area cacat, (g) pass/fail decision.
...
Organisasi disabilitas memerlukan alat bantu membaca. Buatlah: (a) deteksi area teks dalam gambar (text detection), (b) segmentasi teks dari background, (c) preprocessing untuk OCR, (d) integrasi dengan Tesseract OCR, (e) text-to-speech (opsional), (f) support berbagai layout buku, (g) batch processing halaman.
...
Pusat kebugaran ingin tool penghitung repetisi otomatis. Buatlah: (a) deteksi orang (YOLO), (b) pose estimation atau instance segmentation, (c) analisis gerakan (naik/turun) dari posisi bounding box, (d) counting repetisi, (e) real-time dari webcam, (f) timer dan rest counter, (g) statistik per sesi.

### Soal 10: Fashion Item Classifier dan Recommender
E-commerce fashion memerlukan tool klasifikasi produk. Buatlah: (a) klasifikasi jenis pakaian (10 kategori — dari Fashion MNIST atau custom), (b) transfer learning, (c) segmentasi pakaian dari background, (d) ekstraksi warna dominan, (e) rekomendasi sederhana (kemiripan fitur), (f) interface upload foto, (g) top-5 rekomendasi.
...
- **Format**: ZIP — `NIM_Nama_Project05.zip`
