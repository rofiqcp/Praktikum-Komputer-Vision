...
## 5.4 IRLS (Iteratively Reweighted Least Squares)
...
## 5.8 Fitting Kontur dan Template Matching
...
### Konsep
K-Means mengelompokkan data menjadi K cluster berdasarkan jarak ke centroid. Dalam computer vision, digunakan untuk color quantization, segmentasi, dan vector quantization.

### Algoritma
1. Inisialisasi K centroid secara random (atau k-means++).
2. **Assign**: Setiap data point ke cluster centroid terdekat.
3. **Update**: Centroid = rata-rata semua anggota cluster.
...
### Aplikasi dalam Fitting
- **Color quantization**: Reduksi jumlah warna gambar.
- **Segmentasi**: Memisahkan foreground/background berdasarkan warna.
- **BOW (Bag of Words)**: Clustering deskriptor fitur untuk image retrieval.

### Memilih K
- **Elbow method**: Plot inertia vs K, pilih "siku" pada kurva.
- **Silhouette score**: Ukuran seberapa baik sampel cocok dengan clusternya.

---

## 5.10b Kalman Filter Dasar

### Konsep
Kalman filter memprediksi state sistem dinamis dari pengukuran yang noisy. Terdiri dari dua tahap: **predict** (extrapolasi state) dan **correct** (update dengan pengukuran baru).

### Model State-Space
$$x_k = A \cdot x_{k-1} + B \cdot u_k + w_k$$
$$z_k = H \cdot x_k + v_k$$

- $A$: Transition matrix (model gerak).
- $H$: Measurement matrix (apa yang diamati).
- $w_k, v_k$: Process noise dan measurement noise.
...
# Loop: predict → correct
predicted = kf.predict()
estimated = kf.correct(measurement)
```

### Aplikasi
- **Object tracking**: Memprediksi posisi objek ketika deteksi gagal (occlusion).
- **Noise filtering**: Menghaluskan trajectory dari deteksi yang noisy.
- **Sensor fusion**: Menggabungkan data dari multiple sensor.

---

## 5.10c Convex Hull dan Shape Fitting

### Convex Hull
Convex hull adalah poligon terkecil yang melingkupi semua titik. Berguna untuk analisis bentuk objek.
...
### Convexity Defects
Mendeteksi cekungan (defects) antara convex hull dan kontur asli:
```python
...
| Fungsi | Deskripsi |
|--------|-----------|
| `cv2.boundingRect(c)` | Bounding rectangle tegak |
| `cv2.minAreaRect(c)` | Rotated minimum area rectangle |
| `cv2.minEnclosingCircle(c)` | Lingkaran terkecil yang melingkupi |
| `cv2.fitEllipse(c)` | Fitting ellips ke kontur |
| `cv2.fitLine(c, distType, ...)` | Fitting garis dengan berbagai norm |

### cv2.fitLine dengan Berbagai Norm
```python
# distType: cv2.DIST_L2 (least squares), cv2.DIST_L1, cv2.DIST_HUBER
line = cv2.fitLine(points, cv2.DIST_HUBER, 0, 0.01, 0.01)
```
- **DIST_L2**: Standard least squares, sensitif terhadap outlier.
- **DIST_L1**: Lebih robust, median-like.
- **DIST_HUBER**: Kombinasi L1 dan L2, robust dan smooth.
...
| K-Means | Clustering data: color quantization, segmentasi |
| Kalman Filter | Predict-correct untuk tracking di noise |
| Convex Hull | Shape analysis: hull, defects, bounding shapes |
| cv2.fitLine | Fitting garis dengan norm L1, L2, Huber |
