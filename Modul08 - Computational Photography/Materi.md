# MATERI MODUL 8: COMPUTATIONAL PHOTOGRAPHY
...
### 8.1 Perbedaan dengan Global Histogram Equalization
...
### 8.3 Penerapan pada LAB Color Space
...
## 16b. Seamless Cloning (Poisson Blending)

### Konsep
Seamless cloning menggabungkan objek dari satu gambar ke gambar lain menggunakan Poisson equation. Alih-alih menyalin piksel langsung, metode ini menyamakan **gradient** di boundary sehingga objek menyatu natural dengan background baru.

### Prinsip Matematika
Minimisasi:
$$\min_f \iint_\Omega |\nabla f - \mathbf{v}|^2 \quad \text{s.t.} \quad f|_{\partial\Omega} = f^*|_{\partial\Omega}$$

- $\Omega$: Area yang di-clone.
- $\mathbf{v}$: Guidance field (gradient dari sumber).
- $f^*$: Nilai piksel di boundary (dari gambar tujuan).

### Implementasi OpenCV
```python
# src: gambar sumber, dst: gambar tujuan, mask: area yang di-clone
# center: posisi pemasangan di gambar tujuan
...
| Mode | Flag | Deskripsi |
|------|------|-----------|
| Normal Clone | `cv2.NORMAL_CLONE` | Transfer gradient sumber sepenuhnya |
| Mixed Clone | `cv2.MIXED_CLONE` | Gabungan gradient sumber dan tujuan |
| Monochrome Transfer | `cv2.MONOCHROME_TRANSFER` | Hanya luminance, tanpa warna |

### Aplikasi
- Memindahkan objek antar foto secara natural.
- Menghapus teks/watermark dengan clone dari area sekitar.
- Menambahkan elemen kreatif ke komposisi foto.

---

## 16c. Image Colorization dengan DNN

### Konsep
Image colorization menggunakan deep learning untuk memprediksi warna (chrominance) dari gambar grayscale (luminance). Model belajar mapping L→ab di ruang warna CIE LAB dari dataset gambar berwarna.

### Arsitektur (Zhang et al. 2016)
1. Input: Channel L (luminance) dari gambar grayscale.
2. CNN memprediksi distribusi probabilitas untuk channel ab.
3. Ambil prediksi ab (mean atau mode dari distribusi).
4. Gabungkan L asli + ab prediksi → konversi ke BGR.

### Implementasi OpenCV DNN
```python
net = cv2.dnn.readNetFromCaffe(prototxt, caffemodel)
# Preprocessing di LAB space
lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
L = lab[:,:,0]
# Resize L, normalize, forward pass
blob = cv2.dnn.blobFromImage(L_resized, scalefactor=1.0, size=(224,224), mean=50)
net.setInput(blob)
ab_pred = net.forward()[0,:,:,:].transpose((1,2,0))
# Resize ab ke ukuran asli, gabungkan dengan L, konversi ke BGR
```

### Limitasi
- Warna ambigu: Objek yang bisa berwarna apa saja (bola, mobil) mungkin salah prediksi.
- Bias dataset: Langit selalu biru, rumput selalu hijau.
- Tidak ada ground truth unik: Satu gambar grayscale bisa memiliki banyak kemungkinan warna valid.

---

## 16d. Photo Dehazing (Dark Channel Prior)

### Konsep
Gambar berkabut dapat dimodelkan sebagai:
$$I(x) = J(x) \cdot t(x) + A \cdot (1 - t(x))$$

- $I(x)$: Gambar berkabut (observed).
- $J(x)$: Gambar bersih (scene radiance).
- $t(x)$: Transmission map (seberapa banyak cahaya sampai ke kamera).
- $A$: Atmospheric light (warna kabut).

### Dark Channel Prior (He et al. 2009)
Observasi: Di gambar bersih (tanpa kabut), hampir setiap patch memiliki setidaknya satu channel yang sangat gelap (mendekati 0).

$$J^{dark}(x) = \min_{c \in \{R,G,B\}} \left( \min_{y \in \Omega(x)} J^c(y) \right) \approx 0$$

### Pipeline Dehazing
1. **Estimasi dark channel** dari gambar berkabut.
2. **Estimasi atmospheric light** A dari piksel brightest di dark channel.
3. **Estimasi transmission** $t(x)$: area berkabut memiliki dark channel tinggi.
4. **Refine transmission** menggunakan guided filter (edge-preserving).
5. **Recover scene**: $J(x) = \frac{I(x) - A}{\max(t(x), t_0)} + A$

### Implementasi Sederhana
```python
# Dark channel
dark = np.min(img, axis=2)  # minimum antar channel
dark = cv2.erode(dark, np.ones((15,15)))  # minimum dalam patch

# Atmospheric light
flat = img.reshape(-1, 3)
top_indices = dark.flatten().argsort()[-int(0.001*len(dark.flatten())):]
A = flat[top_indices].mean(axis=0)

# Transmission
t = 1 - 0.95 * np.min(img / A, axis=2)
t = cv2.ximgproc.guidedFilter(img[:,:,0], t.astype(np.float32), 60, 1e-3)

# Recovery
J = (img - A) / np.maximum(t[:,:,np.newaxis], 0.1) + A
```
